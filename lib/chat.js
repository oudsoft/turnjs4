...
;(function($, window, document, undefined) {
	...
	var settings = {
		...
		background: '#47b403',
		welcome: 'Welcome ';
		welcomeMsg: 'customer team will contact you as soon as possible.',
	};
	...
	var Chat = {
		...
		startChat: function(event) {
			...
			if(username) {
				...
				Chat.createNewChatRoom(username).done(function() {
					...
					$('#sendMessage').on('click', {name: username}, settings.sendMessage);
				});
			} else {
		...
		createNewChatRoom: function(username) {
			...
			$('.chat-content-page-one').fadeOut(function() { 
				...
				settings.registerNewRoom(username).then((some)=>{
					dfd.resolve(some);
				});				
			});
		...
		*/
		...
		welcomeMessage: function(username) {
			...
			$('body').find('#messageContainer').html('<h5>' + settings.welcome + username +',</h5><p style="position:relative; width: 300px;">'+ settings.welcomeMsg +'</p>');
		},
